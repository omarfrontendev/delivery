...
class ViewOrder extends Component {
	...
	__pickedUp = () => {
		...
		db.collection("orders").doc(this.props.single_delivery_order.id.toString()).update({
			orderstatus_id: '4'
		})
	...
	__getDirectionToUser = (user_order_loaction) => {
		// http://maps.google.com/maps?q=24.197611,120.780512
		console.log(user_order_loaction);
		try {
			...
			const directionUrl =
				...
				"," +
				JSON.parse(user_order_loaction).long;
			window.open(directionUrl, "_blank");
		} catch {
			JSON.parse(user_order_loaction);
															...
															<div className="list-timeline-content">
																...
																<div>
																	<button
																		...
																		onClick={() =>
																			this.__getDirectionToUser(order.address_id)
																		}
																	>
																		<i className="si si-action-redo mr-5" />
																		{localStorage.getItem(
